...
int main(int argc, char **argv) {
    ...
    types_list[0]=MPI_DOUBLE;
    types_list[1]=MPI_CHAR;
    MPI_Type_create_struct(2,lengths,displacements,types_list,&typ_atom_mpi_notresized);
    if(do_it_unsafe) {
        typ_atom_mpi=typ_atom_mpi_notresized;
    }
    else {
        MPI_Type_create_resized(typ_atom_mpi_notresized,(MPI_Aint) 0,
                (MPI_Aint) sizeof(atom[0]),&typ_atom_mpi);
    }
    ...
    if(rank==0) {
        ...
        MPI_Send(&atom[0].xat,2,typ_atom_mpi,1,11,MPI_COMM_WORLD);
    }
    else {
        MPI_Recv(&atom[0].xat,2,typ_atom_mpi,0,11,MPI_COMM_WORLD,&status);
