program Jacobi
    ...
    do istep=0,nstep
        ...
        if(nrank>1) then
            call MPI_Sendrecv(p2(is),1,MPI_DOUBLE_PRECISION,irank_l,11,p2(ie+1),1,MPI_DOUBLE_PRECISION,irank_r,11,MPI_COMM_WORLD,status,ierr)
            call MPI_Sendrecv(p2(ie),1,MPI_DOUBLE_PRECISION,irank_r,22,p2(is-1),1,MPI_DOUBLE_PRECISION,irank_l,22,MPI_COMM_WORLD,status,ierr)
        endif
