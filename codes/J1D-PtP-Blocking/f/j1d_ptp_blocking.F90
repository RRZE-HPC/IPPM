program J1D_PtP_Blocking
    ...
    if(irank_r>=nrank) irank_r=FIXME
    ...
    do istep=0,nstep
        ...
        if(nrank>1) then
            ...
            call MPI_RECV(p2(ie+1),1,MPI_DOUBLE_PRECISION,FIXME,11,MPI_COMM_WORLD,status,ierr)
            call MPI_SEND(FIXME,1,MPI_DOUBLE_PRECISION,irank_r,22,MPI_COMM_WORLD,ierr)
            call MPI_RECV(FIXME,1,MPI_DOUBLE_PRECISION,irank_l,22,MPI_COMM_WORLD,status,ierr)
        endif
