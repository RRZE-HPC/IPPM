...
int main(int argc,char *argv[]) {
    ...
    for(istep=0;istep<=nstep;istep++) {
        ...
        if(nrank>1) {
            ...
            MPI_Recv(&wa[nl-1],1,MPI_DOUBLE,FIXME,11,MPI_COMM_WORLD,&status);
            MPI_Send(FIXME,1,MPI_DOUBLE,irank_r,22,MPI_COMM_WORLD);
            MPI_Recv(FIXME,1,MPI_DOUBLE,irank_l,22,MPI_COMM_WORLD,&status);
        }
        std::swap(pot,wa);
    }
