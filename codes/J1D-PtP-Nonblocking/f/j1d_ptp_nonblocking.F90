program J1D_PtP_Nonblocking
    ...
    do istep=0,nstep
        ...
        p2(ie)=0.5d0*(p1(ie+1)+FIXME)
        if(nrank>1) then
            call MPI_ISEND(p2(is  ),1,MPI_DOUBLE_PRECISION,irank_l,11,MPI_COMM_WORLD,req(1),ierr)
            call MPI_IRECV(p2(ie+1),1,MPI_DOUBLE_PRECISION,irank_r,11,MPI_COMM_WORLD,req(FIXME),ierr)
            call MPI_FIXME(p2(ie  ),1,MPI_DOUBLE_PRECISION,irank_r,22,MPI_COMM_WORLD,req(FIXME),ierr)
            call MPI_FIXME(p2(is-1),1,MPI_DOUBLE_PRECISION,irank_l,22,MPI_COMM_WORLD,req(FIXME),ierr)
        endif
        call sub_without_endpoints(is,ie,p1,p2)
        if(nrank>1) then
            call MPI_WAITALL(FIXME,FIXME,statuses,ierr)
        endif
...
end program J1D_PtP_Nonblocking
