...
int main(int argc,char *argv[]) {
    ...
    for(istep=0;istep<=nstep;istep++) {
        ...
        wa[nl-2]=0.5*(pot[nl-2+1]+FIXME);
        if(nrank>1) {
            MPI_Isend(&wa[1   ],1,MPI_DOUBLE,irank_l,11,MPI_COMM_WORLD,&req[0]);
            MPI_Irecv(&wa[nl-1],1,MPI_DOUBLE,irank_r,11,MPI_COMM_WORLD,&req[FIXME]);
            MPI_FIXME(&wa[nl-2],1,MPI_DOUBLE,irank_r,22,MPI_COMM_WORLD,&req[FIXME]);
            MPI_FIXME(&wa[0   ],1,MPI_DOUBLE,irank_l,22,MPI_COMM_WORLD,&req[FIXME]);
        }
        for(i=2;i<nl-2;i++) wa[i]=0.5*(pot[i+1]+pot[i-1]);
        if(nrank>1) {
            MPI_Waitall(FIXME,FIXME,statuses);
        }
        std::swap(pot,wa);
    }
