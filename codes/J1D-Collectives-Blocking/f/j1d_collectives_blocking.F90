program Jacobi
    ...
    if(restart) then
        ...
        call MPI_FIXME(pot_global,n_loc_ext,MPI_DOUBLE_PRECISION, &
            pot(is),FIXME,MPI_DOUBLE_PRECISION,FIXME,MPI_COMM_WORLD,ierr)
        if(nrank>1) then
    ...
    call MPI_FIXME(pot(is),n_loc_ext,MPI_DOUBLE_PRECISION, &
        pot_global,FIXME,MPI_DOUBLE_PRECISION,FIXME,MPI_COMM_WORLD,ierr)
    if(irank==0) then
