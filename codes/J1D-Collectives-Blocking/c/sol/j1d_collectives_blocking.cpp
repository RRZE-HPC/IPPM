...
int main(int argc,char *argv[]) {
    ...
    if(restart) {
        ...
        MPI_Scatter(pot_global,n_loc_ext,MPI_DOUBLE,
            &pot[1],n_loc_ext,MPI_DOUBLE,0,MPI_COMM_WORLD);
        if(nrank>1) {
    ...
    MPI_Gather(&pot[1],n_loc_ext,MPI_DOUBLE,
        pot_global,n_loc_ext,MPI_DOUBLE,0,MPI_COMM_WORLD);
    if(irank==0) {
    ...
    errmax=0.0;
    for(jrank=0;jrank<nrank;jrank++) {
        errmax=std::max<double>(errmax,errmax_all[jrank]);
        gnrm+=gnrm_all[jrank];
    }
    gnrm=pow(gnrm,0.5);
    if(irank==0) {
