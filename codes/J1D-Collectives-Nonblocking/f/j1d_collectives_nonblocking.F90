program J1D_Collective_Nonblocking
    ...
    do istep=0,nstep
        ...
        if(mod(istep,10)==0) then
            if(flag) then
                ...
                enddo
                call MPI_IALLREDUCE(FIXME,FIXME,FIXME,MPI_DOUBLE_PRECISION, &
                    FIXME,MPI_COMM_WORLD,FIXME,ierr)
                flag=.false.
            else
                call MPI_WAIT(FIXME,status_conv,ierr)
    ...
    enddo
    if(.not. flag) then
        call MPI_FIXME(req_conv,status_conv,ierr)
        gmax_global=gmax_global/h**2
    endif
